 ...
 * - extends → : (콜론으로 상속)
 * - List<Comment> → List<CommentsKt> (타입만 변경)
 ...
 */
@Repository
interface CommentRepositoryKt : JpaRepository<CommentsKt, Long> {

    // 특정 게시글의 모든 댓글 조회
    // SELECT * FROM comments WHERE post_id = ?
    fun findByPostId(postId: Long): List<CommentsKt>

    // 특정 작성자의 모든 댓글 조회
    // SELECT * FROM comments WHERE author = ?
    fun findByAuthor(author: String): List<CommentsKt>
}
